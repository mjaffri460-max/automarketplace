...
export default async function PowersportsPage({ searchParams }: PowersportsPageProps) {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
      <h1 className="text-3xl font-bold text-slate-900">Motorcycles, Dirt Bikes &amp; Jet Skis</h1>
      <p className="mt-2 text-base text-slate-600">
        {items.length} {items.length === 1 ? "listing" : "listings"} available to order and ship.
      </p>

      <form method="get" className="mt-6 flex flex-wrap items-end gap-4 rounded-xl border bg-white p-4">
        ...
        <button
          type="submit"
          className="h-11 rounded-md bg-slate-900 px-6 text-base font-semibold text-white hover:bg-slate-700"
        >
          Search
        </button>
      </form>
